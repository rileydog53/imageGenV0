...
| C5 | `_ENTITY_BBOX` table tracks each protein primitive's *default* size — keep in sync if a primitive's default size changes. Phase 4's master preset will centralise this so the table can come from style instead. | `pathway_layout.py:95-99` | **Medium** (lift during Phase 4) |
...
| L9 | `pathway_layout` does not forward a `size` kwarg to entity primitives — they all use primitive defaults. Wire this up alongside Phase 4 style presets. | `pathway_layout.py:36-38` | **Medium** (lift during Phase 4) |
...
- **End of a step:** if you discover a new "out of scope" decision,
  ...
  via one grep.
- **Phase 4 hand-off:** items L9, C5 are explicitly tagged "lift
  during Phase 4." Don't ship Phase 4 without resolving them.
