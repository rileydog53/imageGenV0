...
**Waves completed:** 1 (P3, ST5), 2 (L9, ST2), 3 (L3, L8, L4), 4 (R1, R2, R4), 5 (L15, L18, L19).

**Next recommended wave (Wave 6):** Remaining Medium-priority rendering bugs.
- **L16** (DAG-aware spring layout init) — `pathway_layout.py::_graph_positions`, add topological-rank initial positions for DAGs.
- **L17** (cross-compartment elbow routing) — already implemented via `_orthogonal_waypoints`; verify it handles the cisplatin case correctly or complete any remaining gaps.
- After Wave 6: **L1** (full force-directed routing), **L2** (force-directed label placement).
